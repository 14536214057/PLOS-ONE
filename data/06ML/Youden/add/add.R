...
# 输入区：只改这里
# =======================
setwd("E:\\AAA_SCI\\IPF_A_SC\\10667_35145\\06ML\\Youden\\add")

trainFile <- "data.train.txt"
testFile  <- "data.test.txt"

# 固定分类门槛
fixed_threshold <- 0.563

# 是否做 train/test 子集双向 Q3 靠近
do_q3_align_both <- TRUE

# 是否对 external/test 子集异常值替换为该子集自身均值
do_outlier_replace_test <- TRUE
...
calc_clinical_metrics <- function(y_true, prob, thr){
  ...
  ppv  <- ifelse((TP + FP) == 0, NA, TP / (TP + FP))   # Precision
...
analyze_one_test_subset <- function(
  ...
  pred_out <- data.frame(
    ...
    Pred_Class = ifelse(as.numeric(pred_test) >= fixed_threshold, 1, 0),
...
ggplot(bar_long, aes(x=Metric, y=Value, fill=Cohort)) +
  ...
  labs(
    title = paste0("Clinical metrics by dataset (threshold = ", fixed_threshold, ")"),
    x = "",
    y = "Value"
  ) +
  theme_bw() +
